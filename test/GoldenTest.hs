...
import           Bcc.Crypto.Encoding.Seed
import           Bcc.Crypto.Encoding.BIP39
import           Crypto.Encoding.BIP39.English (english)
import           Bcc.Internal.Compat (fromRight)
...
type GoldenPaperWallet n
    = "bcc" :> "crypto" :> PathParameter "scramble" n
...
type HDWallet n
    = "bcc" :> "crypto" :> "wallet" :> PathParameter "BIP39-" n
...
goldenHDWallet = group $ do
    summary "This test vectors uses the `Bcc.Crypto.Wallet` primitives to produce extended\n\
